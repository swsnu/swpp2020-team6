// user actions
export const GET_USER_AUTH = "GET_USER_AUTH";
export const SIGN_IN_SUCCESS = "SIGN_IN_SUCCESS";
export const SIGN_OUT_SUCCESS = "SIGN_OUT_SUCCESS";
export const SIGN_IN_OUT_FAILURE = "SIGN_IN_OUT_FAILURE";
export const GET_MYPAGE_USER = "GET_MYPAGE_USER";
export const RESET_MYPAGE_USER = "RESET_MYPAGE_USER";

// roadmap actions
export const GET_ROADMAP_SUCCESS = "GET_ROADMAP_SUCCESS";
export const GET_ROADMAP_FAILURE = "GET_ROADMAP_FAILURE";
export const GET_EDIT_ROADMAP_SUCCESS = "GET_EDIT_ROADMAP_SUCCESS";
export const GET_EDIT_ROADMAP_FAILURE = "GET_EDIT_ROADMAP_FAILURE";
export const CREATE_ROADMAP = "CREATE_ROADMAP";
export const EDIT_ROADMAP = "EDIT_ROADMAP";
export const RESET_ROADMAP = "RESET_ROADMAP";
export const RESET_EDIT_ROADMAP = "RESET_EDIT_ROADMAP";
export const DELETE_ROADMAP = "DELETE_ROADMAP";
export const DUPLICATE_ROADMAP = "DUPLICATE_ROADMAP";

export const CREATE_COMMENT_SUCCESS = "CREATE_COMMENT_SUCCESS";
export const EDIT_COMMENT_SUCCESS = "EDIT_COMMENT_SUCCESS";
export const DELETE_COMMENT_SUCCESS = "DELETE_COMMENT_SUCCESS";

export const ROADMAP_LIKE = "ROADMAP_LIKE";
export const ROADMAP_UNLIKE = "ROADMAP_UNLIKE";
export const ROADMAP_PIN = "ROADMAP_PIN";
export const ROADMAP_UNPIN = "ROADMAP_UNPIN";

// search actions
export const GET_SIMPLE_SEARCH_SUCCESS = "GET_SIMPLE_SEARCH_SUCCESS";
export const GET_SIMPLE_SEARCH_FAILURE = "GET_SIMPLE_SEARCH_FAILURE";
export const GET_ADVANCED_SEARCH_SUCCESS = "GET_ADVANCED_SEARCH_SUCCESS";
export const GET_ADVANCED_SEARCH_FAILURE = "GET_ADVANCED_SEARCH_FAILURE";
export const GET_TOP_TAGS_SUCCESS = "GET_TOP_TAGS_SUCCESS";
export const GET_TOP_TAGS_FAILURE = "GET_TOP_TAGS_FAILURE";

export const PROGRESS_CHANGE = "PROGRESS_CHANGE";
export const CHANGE_CHECKBOX = "CHANGE_CHECKBOX";

export const GET_BEST_ROADMAP_SUCCESS = "GET_BEST_ROADMAP_SUCCESS";
export const GET_BEST_ROADMAP_FAILURE = "GET_BEST_ROADMAP_FAILURE";
export const RESET_BEST_ROADMAP = "RESET_BEST_ROADMAP";

export const GET_NEW_ROADMAP_SUCCESS = "GET_NEW_ROADMAP_SUCCESS";
export const GET_NEW_ROADMAP_FAILURE = "GET_NEW_ROADMAP_FAILURE";
export const RESET_NEW_ROADMAP = "RESET_NEW_ROADMAP";

export const GET_RECOMMENDED_ROADMAP_SUCCESS = "GET_RECOMMENDED_ROADMAP_SUCCESS";
export const GET_RECOMMENDED_ROADMAP_FAILURE = "GET_RECOMMENDED_ROADMAP_FAILURE";
export const RESET_RECOMMENDED_ROADMAP = "RESET_RECOMMENDED_ROADMAP";
